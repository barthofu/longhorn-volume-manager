...
class GdapiClient(object):
    ...
    def sanitize_url(self, url):
        new_url = self._url.split("://")[0] + "://" + self._url.split("://")[1].split("/")[0] + "/v1" + url.split("://")[1].split("/v1")[1]
        return new_url
    ...
    def _get_raw(self, url, data=None):
        r = self._get_response(self.sanitize_url(url), data)
        return r.text

    def _get_response(self, url, data=None):
        r = self._session.get(self.sanitize_url(url), auth=self._auth, params=data,
    ...
    def _post(self, url, data=None):
        r = self._session.post(self.sanitize_url(url), auth=self._auth, data=self._marshall(data),
                               headers=self._headers)
        if r.status_code < 200 or r.status_code >= 300:
            self._error(r.text, r.status_code)
    ...
    def _put(self, url, data=None):
        r = self._session.put(self.sanitize_url(url), auth=self._auth, data=self._marshall(data),
                              headers=self._headers)
        if r.status_code < 200 or r.status_code >= 300:
            self._error(r.text, r.status_code)
    ...
    def _delete(self, url):
        r = self._session.delete(self.sanitize_url(url), auth=self._auth, headers=self._headers)
        if r.status_code < 200 or r.status_code >= 300:
            self._error(r.text, r.status_code)
